...
6. 예외가 발생하지 않는 경우 " 정상적으로 처리되었습니다."라는 메세지와 함께 인덱스의 값을 출력한다.
7. 프로그램 종료 메세지를 출력한다.
8. 마이너ㅏ스인덱스는 적용시키지 않는다. -> 사용자 정의 예외 클래슬르 통해서 적용합니다.
    -> NegaviceNUmIndexError이라고 이름 짓고 처리하겠슴.
...
만약 사용자가 잘못된 속성을 입력하면 적절한 예외 처리 메세질 출력하시오,
...
5. 예외가 발생하지 않은 경우 ' 정상적으로 처리되었습니다.' 라는 메세지와 속성값을 출력한닫.
6. 프로그램 종료 메세지를 출력한다.
'''
# 클래스 정의
class Person:
    def __init__(self,name,age):
        ...
        self.age=age
#객체생성
person1=Person("John",30)   #keyword argument로 객체 생성
# try:
#     attribute1 = input("출력할 속성명ㅇ을 입력하세요.>>>")
# except AttributeError as e:
#     print("존재하지 않는 속성명입니다.")
#     print(e)
# except Exception as e:
#     print("예측할 수 없는 예외가 발ㄹ생했습니다.")
#     print(e)
# else:
#     print("정상적으로 처리되었습니다.")
...
'''
getattr(객체명,속성명-> 대입된 ㄷ애가 들어가야함)함수
이상의 코드를 예시로 생각했을 때,
Person클래스 내부에 있는 인스턴스 변수명을 그대로 집어넣는 것은 불가능하고
input을 통해서 받은 str데이터를 대입했을 때 가능

vars(객체명): 객체의 속성명-값을 dictionary 의 키 - 값쌍으로 만들어주는 함수
'''
#객체를 dictionary로 변환
...
# #     print(profile[key])
# if attribute2 in profile:         # profile의 key 증에 attribute2와 일치하는 것이 있는 묻는 조건문
#     print("정상 출력이 가능합니다.")
# else:
#     raise KeyError
