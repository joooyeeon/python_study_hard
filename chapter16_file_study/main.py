...
# print("myFile.txt 파일이 생성되엇습니다.")
#
# file.close()        #open() 때는 파일이 생성되지 않았기 때문에 함수로 호출하지만 담는 것은
                    # 특정한 파일을 닫는 것이기 때문에 method라는 차이가 잇습니다.
'''
    4. with 문
        close() 메서드를 자동으로 호출할 수 있ㄴ도록 하는 명령어
...
#     str=file.read(10)   #size=1 로 1바이트씩 읽어옴.
#     print(str)
...
# file.close()
'''
이상의 코드는 30바이트씩 가지고 오게 되는데, 첫번째 30바이트의 결과값 이후에 자동으로 개행이 일어납니다.(print()함수의 기본 기능
때문에). 그래서 이 부분을 파일과 콘솔에 정보가 동일하도록 end="" 를 적용한 사례입니다.

    2)  readline()메서드
            텍스트 파일을 한 줄 씪 읽어서 처리하는 메서드
            만약 파일이 종료되어 더 읽어들일 데이터가 없다면 빈 문자열("")을 읽어들입니다.
            반복문을 이용해서 여러 번 읽어들여야 할 때 (한줄씩) 파일 전체를 읽을 수 있습니다.
'''
# file=open("2025-02-26.txt","rt")
# str=file.readline()
# print(str)
...
nation리스트의 내용을 이해하여 다음과 같은 nation.txt 파일을 '생성'하시오.
...
USA-Washington D.C
'''
nation=["Greece","Athene","Germany","Berlin","SouthKorea","Seoul","USA","Washington D.C"]
