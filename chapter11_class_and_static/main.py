...
1.클래스 변수
    ...
    인스턴스 변수 : 인스턴스 마다 서로 다른 값을 지니는 변수
...
# # 클래스 변수 확인 방법
# print(man.country)  # 객체명. 클래스 변수명을 통한 접근 가능
# print(Korean.country) # 클래스명. 클래스 변수명을 통한 접근 가능

# 객체명.클래스 변수를 통해서 클래스 변수에 접근이 가능하긴 하지만 클래스 내부의 코드를
# 들여다보기 전까지는 country라는 변수가 클래스 변수인지 인스턴스 변수인지 확인이 불가능하다는 문제가 있음.

# 이상을 이유로 클래스 변수를 확인하고자 할 때는 객체명. 클레스 변수명으로 참조하기 보다는
# 클래스명. 클래스변수명을 통해서 참조하는 것이 바람직함.
'''
...
#
#     #클래스 메서드 정의 법
#     @classmethod        # 이 코드를 명시하면 밑의 코드가 클래스 메서드로 작성됨
#     def trip(cls, travelling_site):     # 그 결과 첫번쨰 매개변수로 self가 아니라 cls가 자동완성
...
# man2 = Korean2()
# man2.trip("일본")
# 클래스 변수 때와 마찬가지로 객체명. 클래스 메서드명()으로 호출가능
...
2. 클래스 메서드
    ...
    호출 방식 :
        ...
        cls를 통해 클래스 변수에만 접근 간으
        
Korean2 클래스에서 정의된 trip() 메서드는 클래스 멧더ㅡ임을 알리는 @classmethod로 시작,
...
정의하는 경우에 적합함. 정적메서드는 클래스에 소속돼이ㅛ지만 인스턴스에는 영향을 주지도 않고
영향을 받지도 않음
'''
# class Korean3:
#     country = "한국"
...
#
#     @staticmethod  # 정적 메서드 선언
#     def slogan2(str_example):  # self, cls가 자동 생성되지 않았습니다. -> 인스턴스 클래스 변수 ㅊ참조 안하니까
...
#
#     def __init__(self):                 # 생성자 / 인스턴스 메서드에 해당함 -> 아무 @가 없으니까여
#         Bag.count += 1                  # 개체를 하나 생성할 때마다 클래스 변수인 count거 1씩 증가함.
#         print("가방이 생상되었습니당. ")   # 인스턴스 메서드가 클래스 변수를 참조할 떄는 클래스명.클래스변수명으로 접근함
#
#     @classmethod
#     def sell(cls):
#         print("가방이 팔렸습니다.")
#         cls.count-=1                   # 클래스 메서드가 클래스 변수에 접근하기 때문에 Bag.count가 아니라 cls.count입니다.
#
#     @classmethod
...
# print(f"현재 가방 재고: {Bag.remain_bag()}")  # 클래스 변수를 getter를 통해 메서드를 경유하여 참조 ->보안상 더 좋습니더

#객체 생성
# bag1=Bag()
# print(f"현재 가장 재고 : {Bag.remain_bag()}")         #클래스 변수를 클래스 메서드를 통해 조작할 수 있습니다.
# bag2=Bag()
# bag3=Bag()
# print(f"현재 가방 재고 : {Bag.remain_bag()}")
# bag1.sell()                                         #인스턴스명.클래스메서드명()으로 호출 -> 가능했엇죠 // 이경우는 사실 조 ㅁ애매한ㄱ[
# print(f"현재 가방 재고 : {Bag.remain_bag()}")
# Bag.sell()                                          # 클래스명.클레스메서드명()으로 호출-> 더 권장되는 방식
# print(f"현재 가방 재고 : {Bag.remain_bag()}")
'''
이상의 코드에서 보면 예시이기 떄문에 적합하다고 보기 힘든데
인스턴스명.sell()로 작성한다면 특정한 가방 인스턴스가 팔렷다고 볼 수 잇기떄문에 가독성이 더 높은 반면
Bag.sell()로 작성하게 된다면 어떤 가방이 팔렸는지는 모르고 그냥 가방 count 변수만 하나 줄었습니다.

저희는 코딩할 때 변수 / 메서드를 인스턴스 수준으로 작성할지 혹은 클래스 수분으로 작성할지에 대한 
고민이 선행됭어야만 할거빈둥
...
3. 다음 코드를 통해서 전체 인구수를 조회할 수 있도록 작성핫.ㅣ오
print(f"전체 인구수 : {Person.get_population()}")

4. 다음과 같은 방법으로 man 인스턴스를 삭제하시오.
del man

5. man 인스턴스가 삭제되면 다음과 같은 메세지를 출력할 수 있도록 소멸자를 정의하시오
RIP 안근수 
'''
# class Person:
#
...
다음 지시 사항을 읽고 각계ㅡ이 매출을 구할 수 있는 Shop 클래스를 작성하시오.

지시사항
1. Shop 클래스는 다음과 같은 변수를 가지고 있다.
    total: 가게 잔체 매출액
    menu_list : {메뉴얼:가격}으로 이루어진 딕셔너리 요소를 가지는 리스트
...
class Shop:
    ...
    def sales(cls,menu_name,quantity):
        ...
        # 생각해보셔야 할 것이 menu_name을 argument로 받앗을 때, 걔가 리스트 내의 요소의 key에 해당하는지
        # 확인해야하고 , 그 key에 맞는 value를 도출해서 quantity와 곱한 결과값을
        ...
        # argument로 menu_name을 받은 뒤에 그게 일치하는 key를 찾아내고, 그것의 value * quantity 해준 값을
        # total에 더해주면 되지 않을까요
Shop.sales2("떡볶이",1)
Shop.sales2("김밥",2)
Shop.sales2("튀김",5)
