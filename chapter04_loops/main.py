...
'''
while 반복문
    while 다음에 나오는 조건문이 참이라면 이하의 실행문이 반복 실행됨(조건문이 False가 될때까지).
...
if문과의 비교 :
    ...
    whiile문의 경우 조건식이 참일 때 실행문이 '반복' 실행되기 때문에
...
1일차부터 5일차까지 반복되는 일수와 1교시부터 3교시까지 반복되는 시간이라는 2 개의 반복 처리 대상을 기준으로
중첩 while문을 작성합니다.
...
'''
# day = 2
# while day < 10:      # 5일차까지만 출력돼야 하니까.
#     hour = 1
#     while hour < 10: # 3일차까지만 출력돼야 하니까.
