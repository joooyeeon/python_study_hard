...
# table = PrettyTable()
# table.field_names=["구분","부모클래스","자식클래스"]
# table.add_row(["의미","상속해주는 클래스","상속받는 클래스"])
# table.add_row(["용어","슈퍼클래스","서브 클래스"])
# table.add_row(["","기반 클래스","파생 클래스"])
# print(table)
'''
상속
1. 상속이란?
    어떤 클래수ㅡ가 가지고 있는 기능을 그대로 물려받아서 사용할 수 있는 클래스를 생성할 수 있는데, 
    클래스의 기능을 물려받을 때 상속받는다는 표현을 사용함.
    상속 관계에 있는 클래스를 표현할 떄 부모(슈퍼)클래스, 자식(서브)클래스라는 용어를 사용함.
...
+------+-------------------+-----------------+
2. 상속 관계 구현
     두 클래스가 상속 관계에 놓이려면 IS-A관계를 성립해야함. IS-A 관계란 "~은 ~이다"로 해석할 수 있는 관계를 의미
     ex)학생은 사람이다.
...
#         self.school=school          #슈퍼클래스에 없는 인스턴스 변수 school으,ㄴ 서브 클래스에서 선언 및 초기화
#     def study(self):
#         print(f"{self.name}은(는) {self.school}에서 공부합니다.")

#객체생서ㅏㅇ
# potter=Student(name="해리포터",school="호그와트")
# potter.eat("감자")       #Student 클래스에는 없는 부모 클래스의 메서드인 eat()을 호출가능
# potter.study()          #자식 클래스의 메서드 호출
'''
3. 서브 클래스의 __init__()

서브 클래스는 슈퍼클래스가 없으면 존재할 수 없습니다. 그러나 서브 클래스의 생성자를 구현할 때는
'반드시 슈퍼클래스의 생성자를 먼저 호출'하는 코드를 작성해야만 합니다.

super-> 슈퍼클래스를 의미, 즉, Stduent의 생성자를 호출하려면 super().__init__(name)에 의해서
슈퍼 클래스인 Person의 생성자가 먼저 호출되면서 슈퍼 클래스의 객체가 '생성'됩니다.
이후 슈퍼 클래스에서 생성된 인스턴스 변수인 name이 서브 클래스로 전달되고, 이후에 서브 클래스에서 school 인스턴스를 선언 및 포초기화하여
저장하면서 서브 클래스의 객체가 생성됩니다.

4. 서브 클래스의 인스턴스 자료형

슈퍼 클래스의 객체는  슈퍼클래스의 인스턴스
하지마 서브 클래스의 객체는 서브 클래스의 인스턴스이면서 슈퍼클래스의 인스턴스
Student 클래스의 객체는 Student의 인스턴스이면서 동시에 Person의 인스턴스라는 의미입니다,

어떤 객체가 특정한 클래스릐 인스턵스인지 아닌지를 확인하기 위해서 사용하는 함수 :isinstance()
...
'''
isinstance()의 리턴값: bool 자료형으로 True/False를 반환함
'''
'''
3. 다음 지시사항을 읽고 Hybrid 클래스를 구현하세요.

지시사항
1. 다음과 같은 슈퍼 클래스 Car을 ㄱ자ㅣ고 있는 Hybrid 클래스를 구현하세요.
2. 서브 클래스 Hybrid는 다음과 같은 특징을 가지고 있습니다.
    최대 배터리 충전량은 30입니다.
    배터리를 충전하는 charge()메서드가 잇습니다. 최대 충전량을 초과하도록 충전할 수 없고,
...
class Hybrid(Car):
    ...
    def hybrid_info(self):
        super().car_info()                      #super().__init__()에서 볼수 잇듯이
        #print(f"현재 주유 상태: {self.oil}")
